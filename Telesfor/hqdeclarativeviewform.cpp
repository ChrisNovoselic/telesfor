...
HQDeclarativeViewForm::HQDeclarativeViewForm (QDeclarativeView *parent) : QDeclarativeView (parent) {
    Ogre::String pathRes = "res/";
    m_strOgrePluginsCfg = pathRes + "plugins.cfg";
    m_strRenderSystemCfg = pathRes + "ogre.cfg";
    m_strOgreResourcesCfg = pathRes + "resources.cfg";
...
//    m_ManagementTreadmill.free_usb_device_descs ();
    ...
    m_pThreadManagementThradmill->stop ();
//    m_pThreadManagementThradmill->deleteLater ();
    delete m_pThreadManagementThradmill;
}
...
{
    cout << "HQDeclarativeViewForm::init ()" << endl;

    int i, j;

    m_pThreadManagementThradmill = new HThreadManagementThreadmill ();
    ...
    QObject::connect (rootObject (), SIGNAL (sgnManagementTreadmill (QString )), m_pThreadManagementThradmill, SLOT (onSgnManagementTreadmill (QString )));

//    QObject::connect (rootObject (), SIGNAL (sgnQueryPulse (QString )), m_pThreadManagementThradmill, SLOT (onSgnQueryPulse (QString )));
    QObject::connect (m_pThreadManagementThradmill, SIGNAL (sgnPulseTreadmill (QVariant)), rootObject (), SLOT (callbackQueryPulse (QVariant)));
    QObject::connect (m_pThreadManagementThradmill, SIGNAL (sgnEnabledButtonTrack (QVariant)), rootObject (), SLOT (onSgnEnabledButtonTrack (QVariant)));
    QObject::connect (m_pThreadManagementThradmill, SIGNAL (sgnCheckedButtonTrack (QVariant)), rootObject (), SLOT (onSgnCheckedButtonTrack (QVariant)));

    QObject::connect (rootObject (), SIGNAL (sgnTrainingStart (double , double, double )), this, SLOT (onSgnTrainingStart (double , double, double )));
    QObject::connect (rootObject (), SIGNAL (sgnTrainingChange (QString, double )), this, SLOT (onSgnTrainingChange (QString, double )));
    ...
    QObject::connect (rootObject (), SIGNAL (sgnTrainingReport (QString , QString, QString)), this, SLOT (onSgnTrainingReport (QString, QString, QString )));

    initOgre ();
//    m_ManagementTreadmill.husb_init ();
    m_pThreadManagementThradmill->start ();
...
//        m_pSynchCoordinates->setPlacementsMovable (); //Задание местоположения движущимся обЪектАм ('trace', 'pole')
        //Здесь дааные еЩё не считанЫ из БД
//        Местоположение б. задавать по сигналу из Qml 'sgnSetPlacemensMovable'
        QObject::connect (rootObject (), SIGNAL (sgnSetPlacementsMovable (QString )), m_pSynchCoordinates, SLOT (onSgnSetPlacementsMovable (QString )));
        QObject::connect (rootObject (), SIGNAL (sgnSetVisibleObjectReality (QString , int , bool)), m_pSynchCoordinates, SLOT (onSgnSetVisibleObjectReality (QString , int , bool)));
        QObject::connect (rootObject (), SIGNAL (sgnMotionTrace ()), m_pSynchCoordinates, SLOT (onSgnMotionTrace ()));

        QObject::connect (rootObject (), SIGNAL (sgnIsCreateObjectReality (QString, int)), this, SLOT (onSgnIsCreateObjectReality (QString, int)));
        QObject::connect (m_pSynchCoordinates, SIGNAL (sgnCreateObjectReality (QVariant , QVariant)), rootObject (), SLOT (onSgnCreateObjectReality (QVariant , QVariant)));
...
//                    itemQmlCVCapture->createWidgetCVCapture (m_ptrViewportGLWidget);
                    ...
                    QObject::connect (rootObject (), SIGNAL (sgnResetPSC (void)), itemQmlCVCapture, SLOT (onSgnResetPSC (void)));
                    QObject::connect (itemQmlCVCapture, SIGNAL (sgnFixedPSCChanged (QVariant)), rootObject (), SLOT (callbackPSCChanged (QVariant)));

                    QObject::connect (rootObject (), SIGNAL (sgnCheckedLatticeLine (int, bool, QString)), itemQmlCVCapture, SLOT (checkedLatticeLine (int, bool, QString)));

                    //Инициализация обЪекта 'QmlCVCapture'
                    if (itemQmlCVCapture->getWidgetCVCapture ()->initAR (m_listptrVideoCapture.at (itemQmlCVCapture->getNumPort ())) == 0) {
...
//                        }
                        ...
                        cout << "QmlCVCapture initAR == -1" << endl;
                        itemQmlCVCapture->setEnabled (false);
                    }
                }
                else
...
int HQDeclarativeViewForm::initOgre (void) {
    ...
    m_OgreRoot = new Ogre::Root (m_strOgrePluginsCfg, m_strOgreResourcesCfg);
...
//    Следы (1, 2) на ИСХОДную позицию
    ...
    float fltDiffZ = 0;
    cout << metaObject ()->className () << "::onSgnReplacmentObject, " << nameObject.toStdString () << ", " << nameItemManagement.toStdString () << endl;

    if ((! nameObject.isEmpty ()) &&
        (! nameObject.isNull ()) &&
        (! nameItemManagement.isEmpty ()) &&
        (! nameItemManagement.isNull ())) {
...
//            m_pSynchCoordinates->calculate (nameObject, nameItemManagement);
        ...
        ; //Нет НИ обЪектА НИ элементА управлениЯ
}
...
void HQDeclarativeViewForm::onSgnTrainingStart (double speed, double tilt, double stepsLength) {
    QDateTime dtCurrent = QDateTime ();
//    мОЖНО ДОБАВИТЬ НОВ. ОБъЕКТ тренировкА
    m_TrainingDesc.start (speed, tilt, stepsLength);
}

void HQDeclarativeViewForm::onSgnTrainingChange (QString nameParam, double valueParam) {
    m_TrainingDesc.addItem (nameParam, valueParam);
}
...
void HQDeclarativeViewForm::onSgnIsCreateObjectReality (QString namePrefix, int num) {
    HObjectReality *pOject = NULL;

//    По 'nameObject' и известному клаССу обЪектА найдЁм САМ обЪект
    QList <HObjectReality *> listItemQmlObjects = rootObject ()->findChildren <HObjectReality *> ();
    for (int i = 0; i < listItemQmlObjects.length (); i ++) {
//        cout << metaObject ()->className () << "::onSgnIsCreateObjectReality: " << listItemQmlObjects.at (i)->objectName ().toStdString () << endl;
        if ((namePrefix + QString::number (num)).compare (listItemQmlObjects.at (i)->objectName ()) == 0) {
            pOject = listItemQmlObjects.at (i);
            break;
        }
        else
            ;
    ...
    if (! pOject == NULL)
        m_pSynchCoordinates->onSgnIsCreateObjectReality (pOject, namePrefix, num);
    else
        ;
}

void HQDeclarativeViewForm::onSgnTrainingReport (QString content, QString lastName, QString dt)
{
    QString qNameFile;
    ofstream fout;
//    string stdNameFile,
//           stdContent = content.toLocal8Bit ().constData (),
//            stdLastName = lastName.toLocal8Bit ().constData (),
//            stdDt = dt.toLocal8Bit ().constData ();
////    std::replace (stdDt.begin (), stdDt.end (), ' ', '_');
//    stdNameFile = stdLastName + "_" + stdDt + ".html";

//    qNameFile = "report's/" + lastName + "_" + dt + ".html";
    qNameFile = "report's/" + dt + ".html";
    qNameFile.replace (" ", "_");

//    fout.open (stdNameFile.c_str (), ios_base::out | ios_base::trunc | ios_base::binary);
    fout.open (qNameFile.toLocal8Bit ().constData (), ios_base::out | ios_base::trunc | ios_base::binary);
//    fout << stdContent;
    fout << content.toLocal8Bit ().constData ();
    fout.flush ();
    fout.close ();

//    QFileInfo qfi (qNameFile);
//    cout << qfi.isFile () << endl;

//    qNameFile = QString::fromLocal8Bit (stdNameFile.c_str ());
//    QDesktopServices::openUrl (QUrl (qNameFile.toLocal8Bit ().constData ()));
//    QDesktopServices::openUrl (QUrl (QString::fromLocal8Bit (stdNameFile.c_str ())));
    QDesktopServices::openUrl (QUrl (qNameFile));
//    cout << stdNameFile << endl;

//    QDesktopServices::openUrl (urlNameFile);
}
