...
//    Свойство для хранения '_id' 2-го из них
    ...
    signal sgnGetPatientRoutine (string key) //Взять заданиЯ для пациЕНЬа из БД
    signal sgnSetPatientRoutine (string routine) //Установить текущее задание (значения ДЛЯ всех 'param' тренировки)
    property variant patientRoutineJSON: undefined
    property variant defaultRoutineJSON: undefined
...
//    Очистить статистику (левая частЬ)
    ...
    onSgnResetStatistic: {
//        var btnPatientRoutine = columnPatientRoutine.getButtonRoutineChecked ()
//        console.debug ("::onSgnResetStatistic:btnPatientRoutine =", btnPatientRoutine)
...
//                    '"' + 'countStage' + '":"' + parseInt (sliderCurrentTrainingStage + 1) + '"' + ',' +
                    '"' + 'countStage' + '":"' + 1 + '"' + ',' +
//                    '"' + 'countRoutine' + '":"' + sliderTrainingRoutine + '",' +
...
//        timerTrackSpeed.start ()
        ...
        if (idTraining.length) {
            var dtNow = new Date (), timeTotal = (sliderPreviousTime + sliderCurrentTime) + ((sliderPreviousTime + sliderCurrentTime) % 60), putData
             ...
             putData = '{' +
                        '"routines":{' +
                            '"' + (sliderTrainingRoutine - 0) + '":{' +
                            '"stageStart":"' + stage + '"' + ',' +
//                            '"countStage":"' + columnPatientRoutine.getButtonRoutineChecked ().routineJSON ["countStage"] + '"' + ',' +
                            '"id_routine":"' + columnPatientRoutine.getButtonRoutineChecked ().routineJSON ["_id"] + '"' + //',' +
                            '}' +
                        '}' + ',' +
                        '"intervals":{' +
                            '"' + (stage - 1) + '":{' +
...
//        timerTrackSpeedCountTick = 0
        ...
        console.debug ("::nextStageTimeDistance...")
    }
...
//        if ((paramTrainingTime.val < (sliderCurrentTime + timerPulse.interval / 1000)) ||
        if ((paramTrainingTime.val < (sliderCurrentTime + sliderQueryDiscretePulse / 1000)))
//                || не учитывать ДИСТАНЦию
//            (paramTrainingDistance.val < (sliderCurrentDistance + (timerPulse.interval / 1000 / 3600 * paramTrackSpeed.val)))) {
            /*(paramTrainingDistance.val < (sliderCurrentDistance + (sliderQueryDiscretePulse / 1000 / 3600 * paramTrackSpeed.val))))*/ {
            bRes = true
        }
        else
            ;

        console.debug ("::isLimit () return =", bRes)

        return bRes
    }

////    Определяет по какой причине увеличился текущий номер тренировки по изменениЮ параметра ???
//    function isParamTrainingChanged (routineJSON, stage) {
//        var bIsParamTrainingChanged = false
//        if ((routineJSON ["speed"] ["value"] [stage.toString ()] === paramTrackSpeed.val) &&
//            (routineJSON ["tilt"] ["value"] [stage.toString ()] === paramTrackTilt.val) &&
//            (routineJSON ["stepsLength"] ["value"] [stage.toString ()] === paramStepsLength.val) &&
//            (routineJSON ["time"] ["value"] [stage.toString ()] === paramTrainingTime.val) &&
//            (routineJSON ["distance"] ["value"] [stage.toString ()] === paramTrainingDistance.val))
//            ;
//        else {
...
//    Счётчик общего НОМЕРа ЗАДАНия
    property int sliderTrainingRoutine: 0
...
//    Счётчик НОМЕРа этапа ТРЕНИРОВКи
    ...
    onSliderCurrentTrainingStageChanged: {
        var btnPatientRoutine = columnPatientRoutine.getButtonRoutineChecked ()
        console.debug ("::onSliderCurrentTrainingStageChanged =", sliderCurrentTrainingStage, btnPatientRoutine, btnPatientRoutine.routineJSON ["countStage"])
        if (! (btnPatientRoutine === undefined)) {
            if (btnPatientRoutine.routineJSON ["_id"].indexOf ("custom") > -1) {
//                if (isParamTrainingChanged (btnPatientRoutine.routineJSON, sliderCurrentTrainingStage - 1) === true) {
                if (! isLimit ()) {
                    if (! (sliderCurrentTrainingStage === 0)) {
                        var routine = btnPatientRoutine.routineJSON
                        routine ["countStage"] ++

                        console.debug ("::onSliderCurrentTrainingStageChanged () routineJSON =", JSON.stringify (routine))

                        routine ["intervals"] [routine ["countStage"] - 1] = routine ["intervals"] ["0"]

                        routine ["intervals"] [routine ["countStage"] - 1] ["speed"] = paramTrackSpeed.val
                        routine ["intervals"] [routine ["countStage"] - 1] ["incline"] = paramTrackTilt.val
                        routine ["intervals"] [routine ["countStage"] - 1] ["stepsLength"] = paramStepsLength.val
                        routine ["intervals"] [routine ["countStage"] - 1] ["duration"] = paramTrainingTime.val
                        routine ["intervals"] [routine ["countStage"] - 1] ["distance"] = paramTrainingDistance.val

//                        console.debug ("::onSliderCurrentTrainingStageChanged () routineJSON =", JSON.stringify (routine))

                        btnPatientRoutine.routineJSON = routine
                        console.debug ("CustomCustomCustomCustomCustomCustomCustomCustomCustom...", routine ["countStage"], btnPatientRoutine.routineJSON ["countStage"])
...
//                    columnTraining.getButtonTrainingRun ().checked = false
                ...
                if (sliderCurrentTrainingStage < btnPatientRoutine.routineJSON ["countStage"]) {
                    sgnSetPatientRoutine (JSON.stringify (btnPatientRoutine.routineJSON))
                }
                else {
                    columnTreadmill.getButtonTrackRun ().checked = false
                }
...
//    обЪединяет их в 'RadioGroup' - в моменТ времени м.б. выбранА ОДНа и ТОЛьКО ОДНа
    ...
    function onSgnEnabledButtonTrack (enabledValue) {
//        Включение/выКЛЮЧение кнопок Старт/Стоп, Обдув
        console.debug ("::onSgnEnabledButtonTrack =", enabledValue)
        columnTreadmill.enabled = enabledValue
    }

    function onSgnCheckedButtonTrack (checkedValue) {
//        Нажать/отжать кнопКУ Старт/Стоп
        console.debug ("::onSgnCheckedButtonTrack =", checkedValue)
        columnTreadmill.getButtonTrackRun ().checked = checkedValue
    }
...
//    signal sgnQueryPulse (string cmd) //Уже нет необходимостИ
    function callbackQueryPulse (pulse) {
        console.debug ("recieve Pulse from treadmill = ", pulse)        

        if (((pulse === NaN) || (pulse === undefined) || (pulse === null) || (pulse === 0) || (pulse.length === 0))) {
            textPulseVal = "0"
        }
        else { //Из строкИ ответа от ДОРОЖКи '*111#35#VALUe*'
            textPulseVal = parseFloat (pulse)
        }

        var interval = 1000, seconds = interval / 1000
...
//    Изменнеие происходит в 'onTriggered' ТАЙМЕРа пульса ('timerPulse')
    ...
    property real sliderDBCurrentPulse: sliderDBDiscretePulse
...
//    Изменнеие происходит в 'onTriggered' ТАЙМЕРа пульса ('timerPulse')
    property real sliderQueryDiscretePulse: 1000
...
//        Дистанция исключенА из параметров ЗАДАНия
...
//    Ширина области для размещения элементов управления (слева или справа)
    ...
    property string nameDB: "treadmill"
    property string typeContent: "json"

    property real coeffTrackSpeed: 1.17
    property real coeffTrackTilt: 1
...
//        БесПОЛЕЗНые вызовы (связывание сигнАЛ-СЛот НЕ произошло ('hqdeclarativeviewform.cpp::init ()'))
        sgnManagementTreadmill ("*105#110" + coeffTrackSpeed + "*")
        sgnManagementTreadmill ("*105#112" + coeffTrackTilt + "*")
...
//        interval: 13
        running: columnTreadmill.getButtonTrackRun ().checked //false //true
        repeat: true
...
//    Таймер перенесён в поток на стороне C++
//    ВСЕ ин(де)крементации выполняются в обработчике 'callbackQueryPulse' сигнала из потокА на стороне C++
//    Timer {
//        id: timerPulse
//        interval: 250
////        interval: 13
...
//    ОбъектЫ с параметрами ЗНАЧений для писка пациента по ВЕСу (МИНим., МАКСим.)
    ...
    HColumnPatientRoutine { id: columnPatientRoutine }
...
//    Столбец - управление РЕЖИМами работы (КАМЕРАми+ПАЦИЕНТы)
    Column {
        id: columnCamera
        spacing: 6
...
//        anchors.topMargin: global_offset
        ...
        //Строка - описание для области управления РЕЖИМами работы
        Row {
            parent: columnCamera
            width: parent.width
...
//                    ScriptHCouchDB.coutJSON (responseJSON ['rows'])
            ...
            ScriptHCouchDB.viewCouchdb (nameDB, typeContent, "patients", "guest", setTextButtonPatient)
        }
    }
            ...
            function setValue (responseText) {
                ...
                for (var i = 0; i < responseJSON ['total_rows'] - offset; i ++) {
                    if (! (responseJSON ['rows'][i.toString ()] === undefined)) {
                        ...
                        coeffStepsLength = responseJSON ['rows'][i.toString ()]['value'] ["coeffStepsLength"]
