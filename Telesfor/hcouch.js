...
function ageOfBirthDay (strBirthDay) {
    console.debug ("hcouch.js::ageOfBirthDay, in =", strBirthDay)
    var dtCurrent = new Date (), dtBirthDay = getDateTime (strBirthDay.split ('T') [0], strBirthDay.split ('T') [1]), retAge
    retAge = dtCurrent.getFullYear () - dtBirthDay.getFullYear ()
    console.debug ("hcouch.js::ageOfBirthDay, dtBirthDay =", dtBirthDay.getFullYear ())
    if (dtCurrent.getMonth () < dtBirthDay.getMonth ())
        retAge --
    else
        ;
...
function getDateTime (dt, tm) {
    dt = dt.split ('-')
    var tmBase = tm.slice (0, 8), tmRelative = tm.slice (8, tm.length - tmBase.length)
    var retDateTime = new Date (0)

    retDateTime.setFullYear (dt [0]); retDateTime.setMonth (dt [1]); retDateTime.setDate (dt [2])
    retDateTime.setHours (tmBase.split (':') [0]); retDateTime.setMinutes (tmBase.split (':') [1]); retDateTime.setSeconds (tmBase.split (':') [2]);
