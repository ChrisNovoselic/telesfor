...
//    property variant objectRowReplace: undefined //стРОКа ПОДМЕНяемАя
    ...
    function getPrecesionToRound () {
        var precesion = 0
        if (object.objectName.indexOf ("Distance") > -1)
            precesion = 2
        else
            if (object.objectName.indexOf ("StepsLength") > -1)
                precesion = 3
            else
                if (object.objectName.indexOf ("Speed") > -1)
                    precesion = 1
                else
                    ;
    ...
    function minValue (val) {
        var precesion = getPrecesionToRound ()

        val = Math.round (val * Math.pow (10, precesion)) / Math.pow (10, precesion)
