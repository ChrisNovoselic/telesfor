...
HOgreWidget::HOgreWidget (QWidget *parent, Ogre::Root *ogreRoot, HSynchCoordinates *pSynchCoordinates, QString postfixName) : QGLWidget (parent) {
...
//    m_ProjectorPosterNode = 0x0;
    ...
    m_InputCameraFile = "cameraParametrs.yml";
        ...
        if (! (key.isEmpty ())) {
            if (m_mapOgreObjectNode [key]) {
                if (m_mapOgreEntity [key]) {
                    pRemoveNode = (Ogre::SceneNode *) m_OgreSceneManager->getRootSceneNode ()->removeChild (m_mapOgreObjectNode [key]);

                    pRemoveNode->detachObject (m_mapOgreEntity [key]);
                    delete pRemoveNode;
                    m_mapOgreObjectNode.remove (key);

//                    Вариант №1
//                    delete m_mapOgreEntity [key];
//                    m_mapOgreEntity.remove (key);

//                    Вариант №2
//                    m_OgreSceneManager->destroyEntity (m_mapOgreEntity [key]);
...
int HOgreWidget::createLattice (Ogre::SceneNode *parentNode, QString nameNode) {
    ...
    unsigned int i = -1, iCountVertical = 13, iCountHorizontal = 13;
    Ogre::Entity *ptrEntity = NULL;
    Ogre::SceneNode *ptrNode = 0x0;

    Ogre::AxisAlignedBox axisAlignedBoxVertical, axisAlignedBoxHorizontal;
    Ogre::Vector3 vec3TranslateConst,
                vec3TranslateVertical, vec3ScaleVertical,
                vec3TranslateHorizontal, vec3ScaleHorizontal;

    vec3TranslateConst = Ogre::Vector3 (0.01, 0.01, 0.01);

    if (nameNode.contains ("Front") == true) {
        vec3TranslateVertical = Ogre::Vector3 (1, 100, 1), vec3ScaleVertical = Ogre::Vector3 (1, 0, 0);
        vec3TranslateHorizontal = Ogre::Vector3 (100, 1, 1), vec3ScaleHorizontal = Ogre::Vector3 (0, 1, 0);
    }
    else {
        if (nameNode.contains ("Rear") == true) {
            vec3TranslateVertical = Ogre::Vector3 (1, 100, 1), vec3ScaleVertical = Ogre::Vector3 (0, 0, 1);
            vec3TranslateHorizontal = Ogre::Vector3 (1, 1, 100), vec3ScaleHorizontal = Ogre::Vector3 (0, 1, 0);
        }
        else {
        }
    }
...
//        m_arNodeLatticeVertical [i]->setScale (m_vec3DOgreScale.x () * 0.1, m_vec3DOgreScale.y (), m_vec3DOgreScale.z () * 0.1);
        m_arNodeLatticeVertical [i]->setScale (vec3TranslateConst * vec3TranslateVertical); ////На 90 вокруг X - растЯнутЫ по ВЫСОТе
...
//        m_arNodeLatticeHorizontal [i]->setScale (m_vec3DOgreScale.x (), m_vec3DOgreScale.y () * 0.1, m_vec3DOgreScale.z () * 0.1);
        m_arNodeLatticeHorizontal [i]->setScale (vec3TranslateConst * vec3TranslateHorizontal); //в плоскости МАРКЕРа - растЯнутЫ по ШИРИНе
...
//            if (i < m_arMarkers.size ()) {
                // set object poses
                try { m_arMarkers [0].OgreGetPoseParameters (positon, orientation); }
                catch (Ogre::Exception &e) {
                    std::cerr << "An exception has occured: " << e.getFullDescription ().c_str () << std::endl;
                    return ;
                }
...
//                ; //Счётчик 'i' МЕНьШе числа РАСПОЗНАНных МАРКЕРов
            ...
            if (m_shCountEmptyArrayMarkers > 6) {
                for (i = 0; i < listNameObjectReality.size (); i ++) {
                    strKeyObjectReality = listNameObjectReality.at (i);
