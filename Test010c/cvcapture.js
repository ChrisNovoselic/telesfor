function resizeQmlCVCapture () {
    ...
    var heightQmlCvCapture = rectangleForm.height - 4 * global_offset

    if (cameraBase.visible && cameraFace.visible)
//        if (cameraBase.visible)
    {
        cameraBase.x = posXItem (1, 0)
        cameraBase.move (cameraBase.x + global_offset, cameraBase.y + global_offset);
        cameraBase.size (widthQmlCvCapture - 2 * global_offset, heightQmlCvCapture);
        cameraFace.x = posXItem (1, 1)
        cameraFace.move (cameraFace.x + global_offset, cameraFace.y + global_offset);
    ...
    else
    {
        ...
        camera.move (camera.x + global_offset, camera.y + global_offset);
