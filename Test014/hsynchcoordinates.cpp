...
//                        pItemManagement = pItemsManagement.at (a);
        for (b = 0; b < strListNameAxis.count (); b ++) {
            ...
            if ((strListNameAxis.at (b).contains ("X") == true) && (strListNamePlacement.at (a).contains ("position") == true))
                pManagementSlider->setSliderValue (pManagementSlider->sliderValue () + m_shDirectionTrack * floor ((num - 1) / 2) * 2 * m_fltStepsLength);
            else
                ;
...
QVector3D HSynchCoordinates::getPositionTrace (short num) {
    ...
    vec3DRes.setX (m_fltBeginPositionTrace + m_shDirectionTrack * (num - 1) * m_fltStepsLength);
...
//    float fltLimPos = m_shDirectionTrack * m_iCountOfTrace * m_fltStepsLength + m_fltBeginPositionTrace;
...
//        if ((getValue (objTrace, nameManagementItem, axisDirection) - fltLimPos) < 0) {
        if (abs (getValue (objTrace, nameManagementItem, axisDirection) - m_fltBeginPositionTrace) > m_iCountOfTrace * m_fltStepsLength) {
...
//    cntRequired = floor ((m_fltLengthTrack + 2 * m_fltSpeedTrack + abs (m_fltBeginPositionTrace - m_fltBeginPositionTrack)) / m_fltSpeedTrack);
    cntRequired = floor ((m_fltLengthTrack + 2 * m_fltStepsLength + abs (m_fltBeginPositionTrace - m_fltBeginPositionTrack)) / m_fltStepsLength);
    if (cntRequired % 2 == 1)
        cntRequired ++;
    else
        ;
...
int HSynchCoordinates::getNumTraceOfMaxPosition (void) {
    ...
    for (i = 1; i <= countTrace; i ++) {
        ...
        cout << obj.toStdString () << nameManagementItem.toStdString () << axisDirection.toStdString () << "=" << fltCurrentValue << endl;
...
//    HObjectReality *objTrace = NULL, *objPole = NULL;
    int i = -1; //, countOfTrace = getCountOfTrace (); //, indexMaxValue = -1;
    float posMaxValue;
...
//    for (i = 1; i < countOfTrace; i ++) {
    for (i = 1; i < m_iCountOfTrace; i ++) {
        ...
        initValue ("Trace" + QString::number (indexMaxValue), nameManagementItem, axisDirection, posMaxValue + i * m_shDirectionTrack * m_fltStepsLength);

        initValue ("Pole" + QString::number (indexMaxValue), nameManagementItem, axisDirection, posMaxValue + i * m_shDirectionTrack * m_fltStepsLength);
